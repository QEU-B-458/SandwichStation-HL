...
public sealed class ShowNullSpaceSystem : EntitySystem
{
    ...
    private void OnShutdown(EntityUid uid, ShowNullSpaceComponent component, ComponentShutdown args)
    {
        Toggle(uid, false);
    }
    ...
    {
        if (!HasComp<NullSpaceComponent>(uid) && HasComp<NullSpaceComponent>(args.Target))
            args.Cancel();
    }
}
