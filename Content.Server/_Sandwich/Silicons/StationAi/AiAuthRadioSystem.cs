...
public sealed class AiAuthRadioSystem : EntitySystem
{
    ...
    private void OnAuthModuleUninstalled(EntityUid uid, AiAuthModuleComponent comp, ref AiModuleUninstalledEvent args)
    {
        ...
        var binaryOnly = new HashSet<string> { "Binary" };
    ...
    private void OnBorisRadioResetNeeded(BorisRadioResetNeededEvent args)
    {
        ...
        SetEntityChannels(args.BorgUid, new HashSet<string> { "Binary" });
    }
    ...
    private void SyncRadioFromAuthModule(EntityUid authModuleUid)
    {
        ...
        var channels = new HashSet<string>(keyHolder.Channels);
        channels.UnionWith(keyHolder.IntrinsicChannels);
        channels.Add("Binary");
    ...
    /// </summary>
    private void SyncBorisBorgsOnServer(EntityUid serverUid, HashSet<string> channels)
    {
        ...
        foreach (var moduleEnt in server.ModuleContainer.ContainedEntities)
        {
            ...
            {
                if (Exists(borgUid))
                    SetEntityChannels(borgUid, new HashSet<string>(channels));
            }
        }
    }

    private void SetEntityChannels(EntityUid uid, HashSet<string> channels)
    {
