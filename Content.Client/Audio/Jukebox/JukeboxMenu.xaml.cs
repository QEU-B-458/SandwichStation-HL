...
public sealed partial class JukeboxMenu : FancyWindow
{
    ...
    // Sandwich: Volume slider
    private const float VolumeSliderMax = 100f;
    // End Sandwich
    ...
    public JukeboxMenu()
    {
        ...
        // Sandwich: Volume slider
        _cfg.OnValueChanged(SandwichCCVars.JukeboxVolume, OnStoredVolumeChanged, true);
        // End Sandwich
        ...
        VolumeSlider.Rounded = true;
        VolumeSlider.Value = GetStoredVolume() * VolumeSliderMax;
        UpdateVolumeLabel();
    ...
    {
        base.ExitedTree();
        _cfg.UnsubValueChanged(SandwichCCVars.JukeboxVolume, OnStoredVolumeChanged);
    }

    private void OnStoredVolumeChanged(float volume)
    {
        var sliderValue = Math.Clamp(volume, 0f, 1f) * VolumeSliderMax;

        if (!MathHelper.CloseToPercent(VolumeSlider.Value, sliderValue))
    ...
    {
        var volume = GetSliderVolume();
        _cfg.SetCVar(SandwichCCVars.JukeboxVolume, volume);
        UpdateVolumeLabel();
    }

    private float GetStoredVolume()
    {
        return Math.Clamp(_cfg.GetCVar(SandwichCCVars.JukeboxVolume), 0f, 1f);
    }

    private float GetSliderVolume()
    {
        return Math.Clamp(VolumeSlider.Value / VolumeSliderMax, 0f, 1f);
    }

    private void UpdateVolumeLabel()
    {
        VolumeLabel.Text = Loc.GetString("jukebox-menu-volume", ("percent", (int) MathF.Round(GetSliderVolume() * 100f)));
    }
    // End Sandwich
