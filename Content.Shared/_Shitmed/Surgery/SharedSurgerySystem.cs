...
public abstract partial class SharedSurgerySystem : EntitySystem
{
    ...
    private void OnTargetDoAfter(Entity<SurgeryTargetComponent> ent, ref SurgeryDoAfterEvent args)
    {
        ...
        {
            Log.Warning($"Surgery do-after cancelled/handled for {ToPrettyString(args.User)} surgery={args.Surgery} step={args.Step} cancelled={args.Cancelled} handled={args.Handled}");
            return;
        }

        if (args.Target is not { } target)
        {
            Log.Warning($"Surgery do-after missing target for {ToPrettyString(args.User)} surgery={args.Surgery} step={args.Step}");
            return;
        }

        var selectedPart = args.Part is { } netPart ? GetEntity(netPart) : EntityUid.Invalid;
        var validityTarget = selectedPart.Valid ? selectedPart : target;

        if (!IsSurgeryValid(ent, validityTarget, args.Surgery, args.Step, args.User, out var surgery, out var part, out var step))
        {
            Log.Warning($"Surgery invalid on completion for {ToPrettyString(args.User)} surgery={args.Surgery} step={args.Step} target={ToPrettyString(target)} selectedPart={ToPrettyString(validityTarget)}");
            return;
        }

        if (!PreviousStepsComplete(ent, part, surgery, args.Step))
        {
            Log.Warning($"Surgery previous-steps check failed for {ToPrettyString(args.User)} surgery={args.Surgery} step={args.Step} part={ToPrettyString(part)}");
            return;
        }

        if (!CanPerformStep(args.User, ent, part, step, false))
        {
            Log.Warning($"Surgery can-perform check failed on completion for {ToPrettyString(args.User)} surgery={args.Surgery} step={args.Step} part={ToPrettyString(part)}");
            return;
        }
        ...
        Log.Info($"Surgery step applied: user={ToPrettyString(args.User)} surgery={args.Surgery} step={args.Step} part={ToPrettyString(part)}");
        RefreshUI(ent);
    }
    ...
    private void OnCloseIncisionValid(Entity<SurgeryCloseIncisionConditionComponent> ent, ref SurgeryValidEvent args)
    {
        ...
        // Ordinary closure should be available for a normal incision once the outer wound is prepared.
        if (!HasComp<BleedersClampedComponent>(args.Part) ||
        ...
        // If this incision is also handling a reattached limb or deep internal wound, require the deeper prep too.
    ...
    {
        var bodyDamage = TryComp<DamageableComponent>(args.Body, out var damageable)
            ? _damageable.GetTotalDamage((args.Body, damageable))
            : 0;
        var partDamage = TryComp<DamageableComponent>(args.Part, out var partDamageable)
            ? _damageable.GetTotalDamage((args.Part, partDamageable))
            : 0;
    ...
    private void OnPartRemovedConditionValid(Entity<SurgeryPartRemovedConditionComponent> ent, ref SurgeryValidEvent args)
    {
        ...
        // Reattachment is slot-specific. Once the new part is inserted, the surgery should remain valid long enough
        // to allow the sealing step, but only for the matching part occupying the requested slot.
        if (_body.IsPartSlotEmpty(args.Part, ent.Comp.Connection))
            return;

        var matchingInsertedPart = _body.GetBodyChildrenOfType(args.Body, ent.Comp.Part, symmetry: ent.Comp.Symmetry)
            .Any(part =>
                part.Component.ParentSlot?.Id == ent.Comp.Connection &&
                HasComp<BodyPartReattachedComponent>(part.Id));

        if (!matchingInsertedPart)
            args.Cancelled = true;
    }
