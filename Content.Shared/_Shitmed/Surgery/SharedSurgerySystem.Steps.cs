...
public abstract partial class SharedSurgerySystem
{
    ...
    private bool HasDamageGroup(EntityUid entity, string[] group, out DamageableComponent? damageable)
    {
        ...
        foreach (var damageGroup in _damageable.GetDamagePerGroup((entity, damageableComp)))
        {
            if (group.Contains(damageGroup.Key.Id) && damageGroup.Value > 0)
                return true;
        }
    ...
    private void OnTendWoundsStep(Entity<SurgeryTendWoundsEffectComponent> ent, ref SurgeryStepEvent args)
    {
        ...
        var bonus = ent.Comp.HealMultiplier *
                    _damageable.GetDamagePerGroup((args.Body, damageable)).GetValueOrDefault(new ProtoId<DamageGroupPrototype>(ent.Comp.MainGroup));
    ...
    private void OnAffixPartStep(Entity<SurgeryAffixPartStepComponent> ent, ref SurgeryStepEvent args)
    {
        ...
        if (targetPart != default)
        {
            ...
            _damageable.TryChangeDamage(args.Body, _body.GetHealingSpecifier(targetPart.Component) * 2);
    ...
    private void OnSurgeryTargetStepChosen(Entity<SurgeryTargetComponent> ent, ref SurgeryStepChosenBuiMsg args)
    {
        ...
        {
            Log.Warning($"Surgery step choose invalid: actor={ToPrettyString(user)} surgery={args.Surgery} step={args.Step}");
            return;
        }
        ...
        {
            Log.Warning($"Surgery step choose blocked by previous/complete: actor={ToPrettyString(user)} surgery={args.Surgery} step={args.Step} part={ToPrettyString(part)}");
            return;
        }

        if (!CanPerformStep(user, body, part, step, true, out _, out _, out var validTools))
        {
            Log.Warning($"Surgery step choose CanPerform failed: actor={ToPrettyString(user)} surgery={args.Surgery} step={args.Step} part={ToPrettyString(part)}");
            return;
        }
        ...
        if (validTools?.Count > 0)
        {
            foreach (var (tool, toolSpeed) in validTools)
            {
                ...
                if (usedEv.Cancelled)
                {
                    Log.Warning($"Surgery tool use cancelled: actor={ToPrettyString(user)} surgery={args.Surgery} step={args.Step} tool={ToPrettyString(tool)}");
                    return;
                }
        ...
        var ev = new SurgeryDoAfterEvent(args.Surgery, args.Step, GetNetEntity(part));
        ...
        var interactionTarget = _body.GetInteractionTarget(body, part);
        var usedTool = validTools?.Count > 0 ? validTools.Keys.First() : (EntityUid?) null;
        var doAfter = new DoAfterArgs(EntityManager, user, TimeSpan.FromSeconds(duration), ev, body, interactionTarget, usedTool)
        {
            BreakOnMove = true,
        ...
        Log.Info($"Surgery TryStartDoAfter={started}: actor={ToPrettyString(user)} surgery={args.Surgery} step={args.Step} part={ToPrettyString(part)} duration={duration}");
